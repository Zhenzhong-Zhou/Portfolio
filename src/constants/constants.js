export const projects = [
	{
		title: 'No Milk',
		description: "Using HTML, CSS, and JavaScript to build a basic and simple static web in 2018 " +
			"when I took the course of CMPT281. The web is called No Milk, and it is a basic simple shopping cart web that allows " +
			"users to buy products locally.",
		image: '/images/cmpt281.png',
		tags: ["HTML", "CSS", "JavaScript"],
		source: 'https://github.com/Zhenzhong-Zhou/CMPT281_Project',
		visit: 'https://github.com/Zhenzhong-Zhou/CMPT281_Project',
		id: 0,
	},
	{
		title: 'Tian Mao',
		description: "Using JavaScript, EJS, HTML, CSS, Node.js, Express and MongoDB to build a full stack web programming " +
			"in 2020 when I took the course of CPMT350. The app is called Tian Mao, and it is a simple shopping cart app that " +
			"allows users to buy or sell products through the website.",
		image: '/images/cmpt350_project.png',
		tags: ['EJS', "HTML", "CSS", "Node.js", "Express", "MongoDB"],
		source: 'https://github.com/Zhenzhong-Zhou/CMPT350_Project',
		visit: 'https://cmpt350.herokuapp.com/',
		id: 1,
	},
	{
		title: 'Survival Game',
		description: "Using C# and Unity to build a 2D RPG top-down game and developed by team remotely from home. " +
			"The game is called Survival Game, and it was created by CMPT306 team in 2020. The features of this dungeon game is " +
			"that procedure generate map content, a-star pathfinding and load save game. The goal is that " +
			"users have to survive and fight with curve enemies.",
		image: '/images/SurvivalGame.png',
		tags: ["C#", "Unity 2D", 'Git'],
		source: 'https://github.com/selinaw0327/CMPT306',
		visit: 'https://github.com/selinaw0327/CMPT306',
		id: 0,
	},
	{
		title: 'Depth Of Hell',
		description: "Depth of Hell is small boss-rush game developed by a team working " +
			"(Rocket Studios) remotely from home. The Rocket Studios was created by CMPT406 group in 2021. \n" +
			"The features of the game is that the difficulty of fighting monsters " +
			"can be dynamically adjusted with the player's performance. The goal is to gain a sense of progression, " +
			"and have insight into how the fights work. ",
		image: '/images/depth-of-hell.png',
		tags: ['C#', 'Unity 2D', 'Git'],
		source: 'https://github.com/Rocket-Studios/BulletHell',
		visit: 'https://foolmonkey.github.io/DepthsOfHellWeb/',
		id: 3,
	},
	{
		title: 'MERN Memories',
		description: "Using React, Node.js, Express & MongoDB to build a Full Stack MERN Application in 2021. " +
			"The app is called Memories, and it is a simple social media app that allows users to " +
			"post, edit, update and delete interesting events of their daily lives. Other users can like and dislike posts." +
			" It is a self-studying project.",
		image: '/images/memories.png',
		tags: ['MongoDB', 'Express', 'React', 'Node'],
		source: 'https://github.com/Zhenzhong-Zhou/Memory_Project',
		visit: 'https://timeline-memories.netlify.app',
		id: 4,
	},
	{
		title: 'Admin Dashboard',
		description: "It is an admin panel developed by React, Recharts, Material UI, and CSS. " +
			"Some widgets show some information of sales, new members and latest transactions " +
			"with analytical charts display active users monthly. The analytic chart displays active users monthly. " +
			"It is a self-studying project built in 2021.",
		image: '/images/dashboard.png',
		tags: ['React', 'JavaScript', "Recharts", "Material UI", "CSS"],
		source: 'https://github.com/Zhenzhong-Zhou/Admin_Dashboard',
		visit: 'https://backend-dashboard.netlify.app/',
		id: 5,
	},
	{
		title: 'Filmpire',
		description: "Build a comprehensive movie application where users can discover titles within " +
			"specific categories and genres, save them to watchlists, view trailers for upcoming films, " +
			"and access information about the cast, synopsis, and more. It is a self-studying project built in 2022.",
		image: '/images/filmpire.png',
		tags: ['React', 'JavaScript', 'Material UI','Redux', 'Alan AI'],
		source: 'https://github.com/Zhenzhong-Zhou/Filmpire',
		visit: 'https://movie-library-application.netlify.app/',
		id: 6,
	},
	{
		title: 'Blue Boy Adventure',
		description: "Build a comprehensive movie application where users can discover titles within " +
			"specific categories and genres, save them to watchlists, view trailers for upcoming films, " +
			"and access information about the cast, synopsis, and more. It is a self-studying project built in 2022.",
		image: '/images/Blue_boy.png',
		tags: ['React', 'JavaScript', 'Material UI','Redux', 'Alan AI'],
		source: 'https://github.com/Zhenzhong-Zhou/RPG_2D_Adventure',
		visit: '/game/RPG_2D_Adventure.jar',
		id: 7,
	},
	// {
	// 	title: 'NETFLIX',
	// 	description: "",
	// 	image: '/images/NETFLIX.png',
	// 	tags: ['React', 'JavaScript'],
	// 	source: 'https://github.com/Zhenzhong-Zhou/Movie_App',
	// 	visit: 'https://netflix-app-client.netlify.app/register',
	// 	id: 6,
	// },
	// {
	// 	title: 'E-Commerce',
	// 	description: "While building it I use many advanced React & JavaScript topics, " +
	// 		"and use Stripe for card transactions. And trust me, e-commerce applications are impressive.",
	// 	image: '/images/E-Commerce.png',
	// 	tags: ['React', 'JavaScript'],
	// 	source: "https://github.com/Zhenzhong-Zhou/E-Commerce",
	// 	visit: "https://e-commerce-store-practice.netlify.app/",
	// 	id: 7,
	// },
];

export const timeLineData = [
	{year: 2016, text: 'Started my university',},
	{year: 2017, text: 'Majored in Computer Science',},
	{year: 2018, text: 'Started the first static website in my life using html and css ',},
	{year: 2020, text: 'Shared my projects with the world',},
	{year: 2021, text: 'Started for finding jobs',},
];

export const accomplishmentsData = [
	{number: 3, text: 'Certifications',},
	{number: 4, text: 'University Projects'},
	{number: 5, text: 'Deployment Projects',},
	{number: 15, text: 'Self Studying Projects',},
];